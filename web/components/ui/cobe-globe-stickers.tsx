  ...
  useEffect(() => {
    ...
    function init() {
      ...
      globe = createGlobe(canvas, {
        ...
        markerColor: [1.0, 0.52, 0.06],
        glowColor: [0.12, 0.40, 0.95],
        markerElevation: 0,
        markers: markers.map((m) => ({ location: m.location, size: 0.07, id: m.id })),
        arcs: [],
    ...
    <div className={`relative aspect-square select-none ${className}`}>
      ...
      {markers.map((m, i) => (
        <div
          ...
          onPointerDown={handlePointerDown}
          onClick={() => {
            if (!didDragRef.current) onMarkerClick?.(m.id)
          }}
          style={{
            position: "absolute",
            ...
            filter: "url(#sticker-outline) drop-shadow(0 3px 6px rgba(0,0,0,0.6))",
